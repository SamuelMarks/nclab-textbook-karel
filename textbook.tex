...
This time Karel needs to go through several levels of an arcade game
shown in Fig. \ref{fig:newcomm-111}, collect all gems in each level, 
and reach his home which is located at the East end of the top level. 
There is only one opening between each two levels. Initially, 
the robot stands somewhere in the bottom level.
The example can be cloned from user "nclab", its name is "Arcade game".

Clearly, this problem is a bit more complicated than the problems that we 
solved before. The way to solve it is to first 
look for tasks that the robot will be doing in each level. For sure, he will
...
level. Let's introduce a new command for this. We will assume that the 
robot stands at the West end of the level, facing East:
...
\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c}{\PYZsh{} Sweep one level from left to right.}
...
robot stands at the West end of the level, facing East, as required by 
the command {\tt sweep}. Let us call this new command {\tt gowest}\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c}{\PYZsh{} Reach West end of the current level}
...
level. Let us call it {\tt moveup}:\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c}{\PYZsh{} Find the opening and move one }
\PY{c}{\PYZsh{} level up. Assumes that robot is}
\PY{c}{\PYZsh{} at the East end of a level, }
...
\subsection{Making random decisions}

In Level 3 Karel can make random decisions via the command {\tt rand}
