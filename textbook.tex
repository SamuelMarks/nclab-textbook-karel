...
that the robot does something else than you expected.
...
\vspace{-0mm}
\caption{From an algorithm to a program.}
\label{fig:algo-1}
\vspace{-1cm}
\end{center}
\end{figure}
...
\noindent
A {\em computer program} is created by translating an algorithm into a particular 
...
\begin{gbox}
\begin{center}
Of logical and syntax errors, the former are usually much harder to find.
Make sure to spend enough time thinking about the algorithm before you 
start coding!
\end{center}
\end{gbox}
\vspace{4mm}
...
the robot writes an {\em error message} and does nothing.
If our algorithm contains a logical error, then multiple 
